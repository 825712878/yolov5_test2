...
def xyxy2xywh(x):
    ...
    return y
def xyxy2lwth(x):
    # Convert nx4 boxes from [x1, y1, x2, y2] to [x1, w, y1, h] where xy1=top-left, xy2=bottom-right
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 1] = x[:, 2] - x[:, 0]  # width
    y[:, 2] = x[:, 1]            # y1
    y[:, 3] = x[:, 3] - x[:, 1]  # height
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    redundant = True  # require redundant detections 需要冗余的检测
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    # Increment 增加 file or directory path, i.e. runs/exp --> runs/exp{sep}2, runs/exp{sep}3, ... etc.
    path = Path(path)  # os-agnostic 原始怀疑路径
    #  如果输入路径存在且exist_ok=False
    if path.exists() and not exist_ok:
        suffix = path.suffix  # path 的 . …… 后缀 如 .txt
        path = path.with_suffix('')  # 将输入路径后缀改为 空
        # 以f开头表示在字符串内支持大括号内的python 表达式
        # 返回所有匹配的文件路径列表
        dirs = glob.glob(f"{path}{sep}*")  # similar paths
        # re.search()方法扫描整个字符串，并返回第一个成功的匹配。如果匹配失败，则返回None
        # path.stem 去掉后缀
        # path.stem(此处为path去掉后缀)+sep+匹配多个数字  与dirs中的各个文件夹匹配
